...
using SimpleMessages.Web.Models.Account;
using SimpleMessages.Web.Models.Authentication;
using System;
...
{
    // TODO: check each [AllowAnonymous]
    [Authorize]
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            var chk = SignInManager.UserManager.CheckPassword(usr, model.Password);
        ...
        [AllowAnonymous]
        public async Task<ActionResult> ConfirmEmail(Guid userGuid, string code)
        {
            if (userGuid == Guid.Empty || code == null)
            {
                return View("Error");
            }
            var result = await UserManager.ConfirmEmailAsync(userGuid, code);
        ...
        {
            var userGuid = await SignInManager.GetVerifiedUserIdAsync();
            if (userGuid == Guid.Empty)
            {
                return View("Error");
            }
            var userFactors = await UserManager.GetValidTwoFactorProvidersAsync(userGuid);
